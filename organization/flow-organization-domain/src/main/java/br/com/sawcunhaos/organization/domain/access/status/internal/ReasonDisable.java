...
@Builder
@Entity
@Table(name = "SCOS_REASON_DISABLE")
@Auditable
public class ReasonDisable extends BaseEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "REASON_DISABLE_ID")
    private Long id;
    @Column(name = "CODE")
    private String code;
    ...
    /**
     * Ativa o motivo de bloqueio.
     * @throws ScosException SCOS_REASON_DISABLE_003 se já estiver ativo.
     */
    public void activate() {
        if (this.active) {
            throw new ScosException(SCOS_REASON_DISABLE_003);
        }
        this.active = true;
    }

    /**
     * Inativa o motivo de bloqueio.
     * @throws ScosException SCOS_REASON_DISABLE_004 se já estiver inativo.
     */
    public void deactivate() {
        if (!this.active) {
            throw new ScosException(SCOS_REASON_DISABLE_004);
        }
        this.active = false;
    }
}
