...
/** Implementação de {@link EnableAddressTypeUseCase}. */
@Service
@RequiredArgsConstructor
@Slf4j
@Transactional(rollbackFor = ScosException.class)
class EnableAddressTypeUseCaseBean implements EnableAddressTypeUseCase {

    private final AddressTypeService addressTypeService;

    @Override
    public void execute(@NonNull Long id) {
        log.info("Enable AddressType : {}", id);
        addressTypeService.enable(id);
    }
}
