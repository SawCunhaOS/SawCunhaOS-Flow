...
/** Implementação de {@link BlockEmployeeUseCase} — delega a {@link EmployeeService#disable}. */
@Service
@RequiredArgsConstructor
@Slf4j
@Transactional(rollbackFor = ScosException.class)
class BlockEmployeeUseCaseBean implements BlockEmployeeUseCase {

    private final EmployeeService employeeService;

    @Override
    public void execute(@NonNull Long id, @NonNull EmployeeStatusTransitionRequest request) {
        log.info("Block employee: {}", id);
        employeeService.disable(id, request.reasonId(), request.observation());
    }
}
