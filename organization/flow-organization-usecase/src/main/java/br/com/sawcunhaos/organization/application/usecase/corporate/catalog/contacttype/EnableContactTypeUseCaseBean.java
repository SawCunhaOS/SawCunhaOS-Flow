...
/** Implementação de {@link EnableContactTypeUseCase}. */
@Service
@RequiredArgsConstructor
@Slf4j
@Transactional(rollbackFor = ScosException.class)
class EnableContactTypeUseCaseBean implements EnableContactTypeUseCase {

    private final ContactTypeService contactTypeService;

    @Override
    public void execute(@NonNull Long id) {
        log.info("Enable ContactType : {}", id);
        contactTypeService.enable(id);
    }
}
