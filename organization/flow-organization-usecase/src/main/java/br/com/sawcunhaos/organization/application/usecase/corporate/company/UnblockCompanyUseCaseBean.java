...
/** Implementação de {@link UnblockCompanyUseCase} — delega a {@link CompanyService#enable}. */
@Service
@RequiredArgsConstructor
@Slf4j
@Transactional(rollbackFor = ScosException.class)
class UnblockCompanyUseCaseBean implements UnblockCompanyUseCase {

    private final CompanyService companyService;

    @Override
    public void execute(@NonNull Long id, @NonNull CompanyStatusTransitionRequest request) {
        log.info("Unblock company: {}", id);
        companyService.enable(id, request.reasonId(), request.observation());
    }
}
