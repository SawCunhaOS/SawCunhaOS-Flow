...
/** Implementação de {@link BlockCompanyUseCase} — delega a {@link CompanyService#disable}. */
@Service
@RequiredArgsConstructor
@Slf4j
@Transactional(rollbackFor = ScosException.class)
class BlockCompanyUseCaseBean implements BlockCompanyUseCase {

    private final CompanyService companyService;

    @Override
    public void execute(@NonNull Long id, @NonNull CompanyStatusTransitionRequest request) {
        log.info("Block company: {}", id);
        companyService.disable(id, request.reasonId(), request.observation());
    }
}
