...
/** Testes de {@link EnableContactTypeUseCaseBean}: delegação do enable e propagação. */
@ExtendWith(MockitoExtension.class)
class EnableContactTypeUseCaseBeanTest {

    @Mock
    private ContactTypeService contactTypeService;

    @InjectMocks
    private EnableContactTypeUseCaseBean useCase;
    ...
    void executes_delegatesEnableToService() {
        ...
        then(contactTypeService).should().enable(7L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(contactTypeService).enable(any());
