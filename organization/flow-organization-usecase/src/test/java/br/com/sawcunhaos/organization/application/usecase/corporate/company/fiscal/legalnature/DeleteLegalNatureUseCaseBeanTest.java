...
/** Testes de {@link DeleteLegalNatureUseCaseBean}: delegação do delete e propagação. */
@ExtendWith(MockitoExtension.class)
class DeleteLegalNatureUseCaseBeanTest {

    @Mock
    private LegalNatureService legalNatureService;

    @InjectMocks
    private DeleteLegalNatureUseCaseBean useCase;
    ...
    void executes_delegatesDeleteToService() {
        ...
        then(legalNatureService).should().delete(8L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(legalNatureService).delete(any());
