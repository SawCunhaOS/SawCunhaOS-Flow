...
/** Testes de {@link EnableDepartmentUseCaseBean}: delegação do enable e propagação. */
@ExtendWith(MockitoExtension.class)
class EnableDepartmentUseCaseBeanTest {

    @Mock
    private DepartmentService departmentService;

    @InjectMocks
    private EnableDepartmentUseCaseBean useCase;
    ...
    void executes_delegatesEnableToService() {
        ...
        then(departmentService).should().enable(7L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(departmentService).enable(any());
