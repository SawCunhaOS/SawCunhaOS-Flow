...
/** Testes de {@link DisableReasonInactivateUseCaseBean}: delegação do disable e propagação. */
@ExtendWith(MockitoExtension.class)
class DisableReasonInactivateUseCaseBeanTest {

    @Mock
    private ReasonInactivateService reasonInactivateService;

    @InjectMocks
    private DisableReasonInactivateUseCaseBean useCase;
    ...
    void executes_delegatesDisableToService() {
        ...
        then(reasonInactivateService).should().disable(8L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(reasonInactivateService).disable(any());
