...
/** Testes de {@link EnableReasonInactivateUseCaseBean}: delegação do enable e propagação. */
@ExtendWith(MockitoExtension.class)
class EnableReasonInactivateUseCaseBeanTest {

    @Mock
    private ReasonInactivateService reasonInactivateService;

    @InjectMocks
    private EnableReasonInactivateUseCaseBean useCase;
    ...
    void executes_delegatesEnableToService() {
        ...
        then(reasonInactivateService).should().enable(7L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(reasonInactivateService).enable(any());
