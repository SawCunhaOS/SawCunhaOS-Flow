...
/** Testes de {@link InactivateCompanyUseCaseBean}: delegação a {@code companyService.inactivate} e propagação. */
@ExtendWith(MockitoExtension.class)
class InactivateCompanyUseCaseBeanTest {

    @Mock
    private CompanyService companyService;

    @InjectMocks
    private InactivateCompanyUseCaseBean useCase;
    ...
    void executes_delegatesInactivateToService() {
        CompanyStatusTransitionRequest request = CompanyStatusTransitionRequest.builder()
                .reasonId(2L)
                .observation("Encerramento definitivo")
                .build();

        useCase.execute(7L, request);

        then(companyService).should().inactivate(7L, 2L, "Encerramento definitivo");
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(companyService).inactivate(any(), any(), any());

        assertThatThrownBy(() -> useCase.execute(7L, CompanyStatusTransitionRequest.builder().reasonId(2L).build()))
