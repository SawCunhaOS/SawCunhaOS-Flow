...
/** Testes de {@link DisablePositionUseCaseBean}: delegação do disable e propagação. */
@ExtendWith(MockitoExtension.class)
class DisablePositionUseCaseBeanTest {

    @Mock
    private PositionService positionService;

    @InjectMocks
    private DisablePositionUseCaseBean useCase;
    ...
    void executes_delegatesDisableToService() {
        ...
        then(positionService).should().disable(8L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(positionService).disable(any());
