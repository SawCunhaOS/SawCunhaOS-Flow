...
/** Testes de {@link DisableReasonDisableUseCaseBean}: delegação do disable e propagação. */
@ExtendWith(MockitoExtension.class)
class DisableReasonDisableUseCaseBeanTest {

    @Mock
    private ReasonDisableService reasonDisableService;

    @InjectMocks
    private DisableReasonDisableUseCaseBean useCase;
    ...
    void executes_delegatesDisableToService() {
        ...
        then(reasonDisableService).should().disable(8L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(reasonDisableService).disable(any());
