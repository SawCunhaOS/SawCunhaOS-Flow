...
/** Testes de {@link DisableDepartmentUseCaseBean}: delegação do disable e propagação. */
@ExtendWith(MockitoExtension.class)
class DisableDepartmentUseCaseBeanTest {

    @Mock
    private DepartmentService departmentService;

    @InjectMocks
    private DisableDepartmentUseCaseBean useCase;
    ...
    void executes_delegatesDisableToService() {
        ...
        then(departmentService).should().disable(8L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(departmentService).disable(any());
