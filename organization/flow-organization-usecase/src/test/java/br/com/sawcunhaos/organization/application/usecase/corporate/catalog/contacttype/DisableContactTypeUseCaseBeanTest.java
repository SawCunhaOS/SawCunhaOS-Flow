...
/** Testes de {@link DisableContactTypeUseCaseBean}: delegação do disable e propagação. */
@ExtendWith(MockitoExtension.class)
class DisableContactTypeUseCaseBeanTest {

    @Mock
    private ContactTypeService contactTypeService;

    @InjectMocks
    private DisableContactTypeUseCaseBean useCase;
    ...
    void executes_delegatesDisableToService() {
        ...
        then(contactTypeService).should().disable(8L);
    }

    @Test
    @DisplayName("propaga ScosException quando o service falha")
    void whenServiceThrows_propagatesScosException() {
        willThrow(new ScosException()).given(contactTypeService).disable(any());
