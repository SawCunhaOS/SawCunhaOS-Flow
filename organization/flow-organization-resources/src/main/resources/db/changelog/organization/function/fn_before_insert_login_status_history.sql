CREATE OR REPLACE FUNCTION scos.fn_before_insert_login_status_history()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  FROM scos.scos_login_status_history
  WHERE login_id = NEW.login_id
  ORDER BY created_at DESC
  LIMIT 1;

  -- (2) valida que cada motivo informado pertence a ENTITY_TYPE = 'LOGIN'
  IF NEW.reason_activate_id IS NOT NULL THEN
    ...
    IF v_entity_type IS DISTINCT FROM 'LOGIN' THEN
      RAISE EXCEPTION 'reason_activate_id % não pertence a entity_type LOGIN', NEW.reason_activate_id;
    END IF;
  END IF;
  ...
  IF NEW.reason_inactivate_id IS NOT NULL THEN
    ...
    IF v_entity_type IS DISTINCT FROM 'LOGIN' THEN
      RAISE EXCEPTION 'reason_inactivate_id % não pertence a entity_type LOGIN', NEW.reason_inactivate_id;
    END IF;
  END IF;
  ...
  IF NEW.reason_disable_id IS NOT NULL THEN
    ...
    IF v_entity_type IS DISTINCT FROM 'LOGIN' THEN
      RAISE EXCEPTION 'reason_disable_id % não pertence a entity_type LOGIN', NEW.reason_disable_id;
    END IF;
  END IF;
  ...
  IF NEW.reason_enable_id IS NOT NULL THEN
    ...
    IF v_entity_type IS DISTINCT FROM 'LOGIN' THEN
      RAISE EXCEPTION 'reason_enable_id % não pertence a entity_type LOGIN', NEW.reason_enable_id;
    END IF;
  END IF;
