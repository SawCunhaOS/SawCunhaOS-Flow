...
These services build on `scos-foundation`. Apply the `scos-conventions` skill
alongside this one. The web layer follows the **OpenAPI-first delegate pattern**:
the REST contract is defined in `etc/api/organization/*.yml` first, code is
generated by `openapi-generator-maven-plugin`, and delegates implement the
generated `XxxApiDelegate` interface — do NOT write `@RestController` classes
manually. Errors are signaled by throwing `ScosException(ExceptionCode, args...)`
— the foundation's `ExceptionsHandler` is global, so do NOT add a per-project
`@RestControllerAdvice`. Also add the foundation dependencies
(`scos-foundation-utils`, `scos-foundation-exception`, and audit/jdempotent
as needed).
...
`scos-conventions` (delegate pattern, error handling, OpenAPI response contracts)
before producing substantial output.
...
3. Build the web layer thin, in SCOS style (see the `scos-conventions` skill):
   ...
   - Bean Validation (`@Valid`, `jakarta.validation` + the foundation's BR
     ...
     domain output with MapStruct or manual mapping.
   - One use-case call per delegate method; no business logic in delegates.
...
Assess against the `scos-conventions` conformance checklist. The findings that recur most:
