...
/**
 * Teste de integração full-stack do agregado ReasonEnable (shape A — enable/disable).
 * Unicidade por {@code (code, entityType)}; entityType ∈ COMPANY/EMPLOYEE/LOGIN.
 *
 * <p>Seed (após reseed): id=1 COMPANY/AUDIT_CLEARED, id=2 EMPLOYEE/AUDIT_CLEARED,
 * id=3 LOGIN/UNBLOCKED_BY_SUPPORT (todos ativos).
 */
public class ReasonEnableControllerTest extends ScosOrganizationTestUtil {

    private static final String URI = "/api/v1/reason-enable";
    private static final long SEEDED_ID = 1L;
    private static final String SEEDED_CODE = "AUDIT_CLEARED";
    ...
    private static final String DETAIL_NOT_FOUND = "O motivo de desbloqueio informado não existe.";
    private static final String DETAIL_CODE_CONFLICT = "Já existe um motivo de desbloqueio cadastrado com esse código.";
    private static final String DETAIL_ALREADY_ACTIVE = "O motivo de desbloqueio informado já está ativo.";
    private static final String DETAIL_ALREADY_INACTIVE = "O motivo de desbloqueio informado já está inativo.";
    ...
    private static final String CODE_NOT_FOUND = "SCOS_REASON_ENABLE_001";
    private static final String CODE_CONFLICT = "SCOS_REASON_ENABLE_002";
    private static final String CODE_ALREADY_ACTIVE = "SCOS_REASON_ENABLE_003";
    private static final String CODE_ALREADY_INACTIVE = "SCOS_REASON_ENABLE_004";
    ...
    @Test
    @DisplayName("GET /v1/reason-enable — token válido lista os motivos semeados (200)")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable — filtro entityType=EMPLOYEE não retorna os de COMPANY (200)")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable — filtro active=false não retorna o seed ativo (200)")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable — sem token retorna 401")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable — token inválido (fora do JWKS) retorna 401")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable — sem permissão retorna 403 no padrão RFC 9457")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable/{id} — retorna o motivo do seed (200)")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable/{id} — id inexistente retorna 404 SCOS_REASON_ENABLE_001")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable/{id} — sem token retorna 401")
    ...
    @Test
    @DisplayName("GET /v1/reason-enable/{id} — sem permissão retorna 403")
    ...
    @Test
    @DisplayName("POST /v1/reason-enable — payload válido cria e retorna o id (201)")
    void create_withValidToken_returns201() throws Exception {
        mockMvc.perform(post(URI)
                        .headers(httpHeaders(LANGUAGE_PT, BEAR_TOKEN_VALID, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("RENA_NEW1", "Novo motivo", "COMPANY")))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.data.id").exists());
    }

    @Test
    @DisplayName("POST /v1/reason-enable — code/entityType já existente retorna 409")
    ...
    @Test
    @DisplayName("POST /v1/reason-enable — sem code retorna 400 de validação")
    ...
    @Test
    @DisplayName("POST /v1/reason-enable — code vazio retorna 400 de validação")
    ...
    @Test
    @DisplayName("POST /v1/reason-enable — sem token retorna 401")
    void create_withoutToken_returns401() throws Exception {
        mockMvc.perform(post(URI)
                        .headers(httpHeaders(LANGUAGE_PT, null, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("RENA_NOTOK", "Sem token", "COMPANY")))
                .andExpect(status().isUnauthorized());
    }

    @Test
    @DisplayName("POST /v1/reason-enable — sem permissão retorna 403")
    void create_withoutPermission_returns403() throws Exception {
        ...
        mockMvc.perform(post(URI)
                        .headers(httpHeaders(LANGUAGE_PT, BEAR_TOKEN_VALID, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("RENA_NOPERM", "Sem permissão", "COMPANY")))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.status").value(403))
                .andExpect(jsonPath("$.code").value(CODE_ACCESS_DENIED));
    }
    ...
    @Test
    @DisplayName("POST /v1/reason-enable — duas chamadas idênticas são idempotentes (Redis)")
    void create_sameRequestTwice_isIdempotent() throws Exception {
        String body = body("RENA_IDEMP", "Idempotente", "COMPANY");
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id} — payload válido atualiza (204)")
    void update_withValidToken_returns204() throws Exception {
        mockMvc.perform(put(URI + "/{id}", SEEDED_ID)
                        .headers(httpHeaders(LANGUAGE_PT, BEAR_TOKEN_VALID, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("RENA_UPD1", "Atualizado", "COMPANY")))
                .andExpect(status().isNoContent());

        mockMvc.perform(get(URI + "/{id}", SEEDED_ID)
                        .headers(httpHeaders(LANGUAGE_PT, BEAR_TOKEN_VALID, MediaType.APPLICATION_JSON_VALUE)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.code").value("RENA_UPD1"))
                .andExpect(jsonPath("$.data.description").value("Atualizado"));
    }

    @Test
    @DisplayName("PUT /v1/reason-enable/{id} — code/entityType de outro registro retorna 409")
    void update_duplicateCode_returns409() throws Exception {
        mockMvc.perform(put(URI + "/{id}", SEEDED_ID)
                        .headers(httpHeaders(LANGUAGE_PT, BEAR_TOKEN_VALID, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("AUDIT_CLEARED", "Colidindo", "EMPLOYEE")))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.status").value(409))
                .andExpect(jsonPath("$.code").value(CODE_CONFLICT))
                .andExpect(jsonPath("$.detail").value(DETAIL_CODE_CONFLICT));
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id} — id inexistente retorna 404")
    void update_notFound_returns404() throws Exception {
        mockMvc.perform(put(URI + "/{id}", NONEXISTENT_ID)
                        .headers(httpHeaders(LANGUAGE_PT, BEAR_TOKEN_VALID, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("RENA_NOPE", "Inexistente", "COMPANY")))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404))
                .andExpect(jsonPath("$.code").value(CODE_NOT_FOUND))
                .andExpect(jsonPath("$.detail").value(DETAIL_NOT_FOUND));
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id} — sem code retorna 400 de validação")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id} — sem token retorna 401")
    void update_withoutToken_returns401() throws Exception {
        mockMvc.perform(put(URI + "/{id}", SEEDED_ID)
                        .headers(httpHeaders(LANGUAGE_PT, null, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("RENA_NOTOK2", "Sem token", "COMPANY")))
                .andExpect(status().isUnauthorized());
    }

    @Test
    @DisplayName("PUT /v1/reason-enable/{id} — sem permissão retorna 403")
    void update_withoutPermission_returns403() throws Exception {
        ...
        mockMvc.perform(put(URI + "/{id}", SEEDED_ID)
                        .headers(httpHeaders(LANGUAGE_PT, BEAR_TOKEN_VALID, MediaType.APPLICATION_JSON_VALUE))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body("RENA_NOPERM2", "Sem permissão", "COMPANY")))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.status").value(403))
                .andExpect(jsonPath("$.code").value(CODE_ACCESS_DENIED));
    }
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/enable — registro inativo é reativado (204)")
    void enable_inactive_returns204() throws Exception {
        long id = create("RENA_ENA", "Para reativar", "COMPANY");
        disable(id);
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/enable — já ativo retorna 422 SCOS_REASON_ENABLE_003")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/enable — id inexistente retorna 404")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/enable — sem token retorna 401")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/enable — sem permissão retorna 403")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/disable — registro ativo é inativado (204)")
    void disable_active_returns204() throws Exception {
        long id = create("RENA_DIS", "Para inativar", "COMPANY");
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/disable — já inativo retorna 422 SCOS_REASON_ENABLE_004")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/disable — id inexistente retorna 404")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/disable — sem token retorna 401")
    ...
    @Test
    @DisplayName("PUT /v1/reason-enable/{id}/disable — sem permissão retorna 403")
