...
- [x] Task 1: Estender `SCOS_LOGIN_APPROVAL_REQUEST` — coluna `REQUEST_TYPE` (AC: 1)
  - [x] Novo changeSet em `organization/flow-organization-resources/src/main/resources/db/changelog/organization/v1.0.0/tables/scos_login_approval_request.yml` (mesmo arquivo, novo `changeSet` ao final — **nunca** editar o `changeSet` de 2026-07-18 já aplicado):
    ```yaml
      - changeSet:
  ...
  - [x] Novo `changeSet` em `organization/flow-organization-resources/src/main/resources/db/changelog/organization/checks/checks.yml` (mesmo arquivo, ao final, **não** editar os `ALTER TABLE` de migrações já aplicadas):
    ```yaml
      - changeSet:
  ...
  - [x] **Pré-requisito descoberto nesta análise, sem o qual `OutboxEvent` não grava:** `OutboxEvent.topic` é `@ManyToOne` `NOT NULL` pra `OutboxTopic` (FK) — não existe hoje nenhuma linha semeada em `SCOS_OUTBOX_TOPIC` pra Keycloak, em nenhum lugar do projeto (`etc/database/seed_data.sql` auditado, sem menção). Como esta story (Task 8) é a **primeira** a de fato inserir um `OutboxEvent`, adicionar em `etc/database/seed_data.sql`:
    ```sql
...
- [x] Task 2: `Department.manager` — campo Java + API de gerenciamento (coluna `MANAGER_ID` já existe no banco desde 2026-07-18, nunca foi exposta) (AC: 2, 8)
  - [x] Em `domain/corporate/department/internal/Department.java`, adicionar:
    ```java
  ...
  - [x] **Sem API pra definir isso, o nível `MANAGER` nunca resolve em produção** (`department.manager` fica sempre `null`) — reaproveitar o endpoint de update já existente em vez de criar um novo:
...
- [x] Task 3: Enums novos do agregado `LoginApprovalRequest` (AC: 1, 2, 4, 7)
  - [x] `domain/access/login/internal/LoginApprovalRequestLevel.java`: `SUPERVISOR, MANAGER, SYSTEM_ACCESS_GROUP` (mesma ordem/nomes do `CHECK chk_login_approval_request_level`).
  - [x] `domain/access/login/internal/LoginApprovalRequestEscalationPolicy.java`: `INDEFINITE, AUTO_CANCEL` (mesmos nomes do `CHECK chk_login_approval_request_policy`).
  - [x] `domain/access/login/internal/LoginApprovalRequestStatus.java`: `PENDING, APPROVED, REJECTED, CANCELLED` (mesmos nomes do `CHECK chk_login_approval_request_status`).
  - [x] `domain/access/login/internal/LoginApprovalRequestType.java`: `CREATE_LOGIN, REACTIVATE_LOGIN, CHANGE_PROFILE` (mesmos nomes do `CHECK` da Task 1).

- [x] Task 4: Entidade `LoginApprovalRequest` + `LoginApprovalRequestRepository` (AC: 1, 4)
  - [x] `domain/access/login/internal/LoginApprovalRequest.java` — espelha exatamente `scos_login_approval_request.yml` (Task 1 inclusa): `id`, `login` (`@ManyToOne` → `Login`), `requestedByLogin` (`@ManyToOne` nullable → `Login`), `decidedByLogin` (`@ManyToOne` nullable → `Login`), `currentLevel` (`LoginApprovalRequestLevel`), `escalationPolicy` (`LoginApprovalRequestEscalationPolicy`), `status` (`LoginApprovalRequestStatus`), `requestType` (`LoginApprovalRequestType`), `isExceptionSelfApproval` (`boolean`), `slaDeadline`/`supervisorNotifiedAt`/`managerEscalatedAt`/`systemGroupEscalatedAt`/`decidedAt` (`Instant` — coluna `TIMESTAMPTZ`, ver regra de tipos temporais do projeto). Estende `BaseEntity`, `@Auditable`.
...
- [x] Task 8: Use Cases — criação injeta `LoginApprovalRequest`; consulta; decisão (aprovar/rejeitar) (AC: 1, 2, 3, 5, 6, 7, 9, 10)
  ...
  - [x] Adicionar 2 métodos novos em `Login.java` (mesmo padrão de `activate()`/`inactivate()`/`disable()`/`enable()` já existentes — validam a transição e devolvem `LoginStatusHistory`, não persistem):
    ```java
...
- [x] Task 9: `BusinessDayCalculator` + job de escalonamento SLA (AC: 1, 4)
  ...
  - [x] Criar `LoginApprovalEscalationJob.java` (`@Component`, `@Scheduled(cron = "...")` — **primeiro uso de `@Scheduled` no projeto inteiro**; confirmar que `@EnableScheduling` está presente em algum `@Configuration` do módulo `boot` — se não estiver, adicionar). Frequência sugerida: a cada 15-30 min (mesma ordem de grandeza do refresh das views de autoridade, 30 min) — **decisão de design a confirmar com o usuário**, o épico só diz "SLA de 1 dia útil", não a frequência do polling.
    ...
    ```
  - [x] Criar `LoginApprovalEscalationJobTest.java` com `Clock.fixed(...)` determinístico (padrão já convencionado, Story 0.2) — sem `sleep`, sem depender de tempo real.

- [x] Task 10: Guarda de escopo (AC: 1-7)
  - [x] **Não** implementar Story 3.3 (`enable`/`unblock` passando pela cadeia), 3.4 (perfil), 3.5 (retorno automático) ou 3.6 (aprovação de Perfil/Resource) — esta story só cobre `requestType=CREATE_LOGIN`. Os outros `requestType` (`REACTIVATE_LOGIN`, `CHANGE_PROFILE`) já entram no `CHECK` (Task 1) para não precisar de outra migração de constraint, mas nenhum código desta story os produz.
  - [x] **Não** implementar o dispatcher/consumidor do Outbox — só a gravação da linha na aprovação (AC 7). Mesma guarda da Story 3.1.
  - [x] **Não** criar calendário de feriados — `BusinessDayCalculator` só pula fim de semana.
  - [x] **Não** implementar Motor de Notificação — AC 6 é só consulta via `GET`.
...
- Regressão descoberta e corrigida: `LoginServiceBean.create()` agora resolve `requestedByLoginId` via `LoginRepository.findByLogin(username autenticado)` — o Login de teste `inside.admin` (usado por `BEAR_TOKEN_VALID`) não existia no seed, quebrando `LoginControllerTest` (Story 3.1). Corrigido semeando `inside.admin` (EMPLOYEE, employee_id=1) só no seed de teste (`setsup_database.sql`) — não em produção, já que é uma identidade sintética do JWT de teste.
- `vw_authority_response`/`vw_login_context` são **materialized views** (refresh só via `pg_cron` em produção) — sem `REFRESH MATERIALIZED VIEW` explícito no seed de teste, `GET /v1/login-approval-requests/system-access-approvers` sempre voltava vazio. Adicionado ao final de `setsup_database.sql`.
- `LoginStatus` do OpenAPI (`ScosOrganization_Login.yml`) não incluía `REJECTED` (só `PENDING_APPROVAL/ACTIVE/BLOCKED/INACTIVE`) — gap da Story 3.1 nunca exercitado até esta story produzir o 1º Login `REJECTED` real; `GET /v1/logins/{id}` quebrava com 500 ao mapear. Corrigido.
- `FindDepartmentUseCaseBean`/`FindAllDepartmentUseCaseBean` construíam o DTO `Department` manualmente sem `managerId` (o `DepartmentMapper` interno já mapeava certo — o bug era só na borda API). Corrigido nos dois use cases.
- 3 permissões novas (`GET_LOGIN_APPROVAL_REQUEST`, `DECIDE_LOGIN_APPROVAL_REQUEST`, `APPROVE_SYSTEM_ACCESS`) adicionadas também a `SCOS_RESOURCE` no seed (produção e teste) — sem isso `system-access-approvers` nunca resolveria ninguém, mesmo com o `SCOS_PROFILE_RESOURCE` cross-join existente.
- Suíte completa (`mvn test`, todos os módulos) verde ao final: domain, usecase, infrastructure (`PermissionsConsistencyTest`) e boot (integração, Testcontainers/Docker) — incluindo `LoginControllerTest`/`DepartmentControllerTest` pré-existentes, sem regressão.
- **Correção pós-review:** `DecideLoginApprovalRequestUseCaseBean` e os demais Use Cases de `access/login` (Find/FindAll/FindSystemAccessApprovers) chamavam `*Repository` diretamente, quebrando o padrão usecase→domain service→repository do projeto. Extraída toda a regra de negócio para um novo domain service `LoginApprovalRequestServiceBean` (specification `LoginApprovalRequestService`, em `domain/access/login/`), e `FindSystemAccessApproversUseCaseBean` passou a usar `AuthorityResponseService.findAllByPermission` (novo método na specification já existente) em vez de `VwAuthorityResponseRepository` direto. `LoginApprovalRequestApiMapper` também parou de receber a entidade `LoginApprovalRequest`/`Employee` diretamente — passou a trabalhar só com o novo DTO `LoginApprovalRequestOutput` (domain), mantendo a fronteira `internal` intacta. Suíte completa revalidada verde após a correção.

### Completion Notes List

- Task 1: `REQUEST_TYPE` adicionado via novo changeSet (não editou o changeSet de 2026-07-18); `CHECK` correspondente; seed de `SCOS_OUTBOX_TOPIC` (`KEYCLOAK_LOGIN_SYNC`/`DIRECT_API`) em produção e teste.
- Task 2: `Department.manager` (campo Java, coluna já existia); `managerId` opcional em `UpdateDepartmentRequest`/`Department` (OpenAPI); `DepartmentServiceBean.update` resolve o Funcionário via `EmployeeQueryRepository` (404 `SCOS_EMPLOYEE_014` se não existir); reaproveita `updateDepartment`, sem rota nova.
- Tasks 3-4: 4 enums + entidade `LoginApprovalRequest` (`escalate`/`approve`/`reject`, guardados por `status==PENDING`) + `LoginApprovalRequestRepository` (`findAllFiltered` por status/loginId, `findAllByStatusAndSlaDeadlineBefore` para o job).
- Task 5: `LoginApprovalChainResolver` como classe utilitária estática (não `@Service`) em `domain/access/login/service`, reaproveitável por `LoginServiceBean`, pelos Use Cases de consulta e pelo job.
- Task 6: 3 permissões novas + mensagens pt/en + entradas em `SCOS_RESOURCE` (seed).
- Task 7: contrato em arquivo próprio (`ScosOrganization_LoginApprovalRequest.yml`, decisão confirmada com o usuário); `VwAuthorityResponseRepository.findAllByPermission` via `@Query` nativa.
- Task 8: `LoginServiceBean.create()` abre a `LoginApprovalRequest` na mesma transação (decisão de design: colocado no domain service, não duplicado em cada Use Case de criação — Epic 4 herda de graça); `Login.approve()/reject()`; `DecideLoginApprovalRequestUseCaseBean` (elegibilidade via `SecurityContextHolder` — mesmo mecanismo do `@PreAuthorize`, não `AuthorityResponseService`/DB); `LoginApprovalRequestDelegate` próprio.
- Task 9: `BusinessDayCalculator` (`@Component`, `java.time.DayOfWeek`); `LoginApprovalEscalationJob` (`@Scheduled` a cada 30 min, decisão confirmada); `@EnableScheduling` adicionado a `ScosOrganizationApplication`.
- Task 10: guarda de escopo respeitada — nenhum código de Story 3.3/3.4/3.5/3.6, dispatcher Outbox, calendário de feriados ou Motor de Notificação foi implementado.
- Todas as 5 decisões de design sinalizadas na story foram confirmadas com o usuário antes da implementação (arquivo OpenAPI separado; `ReasonInactivate` reaproveitado para rejeição; job a cada 30 min; tópico `KEYCLOAK_LOGIN_SYNC`/`DIRECT_API`; `DECIDE_LOGIN_APPROVAL_REQUEST` livre por Perfil).

### File List

**Contrato OpenAPI:**
- `etc/api/organization/ScosOrganization_LoginApprovalRequest.yml` (novo)
- `etc/api/organization/ScosOrganization_Department-Position.yml` (managerId em `Department`/`UpdateDepartmentRequest`)
- `etc/api/organization/ScosOrganization_Login.yml` (`LoginStatus` ganha `REJECTED`)
- `organization/flow-organization-usecase/pom.xml`, `organization/flow-organization-api/pom.xml` (execution do novo spec)
...
- `etc/database/organization/seed_data.sql`
- `organization/flow-organization-boot/src/test/resources/postgresql/setsup_database.sql`

**Domain (`flow-organization-domain`):**
- `domain/corporate/department/internal/Department.java`
- `domain/corporate/department/dto/DepartmentInput.java`, `DepartmentOutput.java`
- `domain/corporate/department/service/DepartmentMapper.java`, `DepartmentServiceBean.java`
- `domain/access/login/internal/Login.java` (`approve`/`reject`)
- `domain/access/login/internal/LoginApprovalRequest.java`, `LoginApprovalRequestRepository.java`, `LoginApprovalRequestPredicates.java` (novos)
- `domain/access/login/internal/LoginApprovalRequestLevel.java`, `LoginApprovalRequestEscalationPolicy.java`, `LoginApprovalRequestStatus.java`, `LoginApprovalRequestType.java` (novos)
- `domain/access/login/internal/VwAuthorityResponseRepository.java` (`findAllByPermission`)
- `domain/access/login/service/LoginServiceBean.java` (`openApprovalRequest`)
- `domain/access/login/service/LoginApprovalChainResolver.java`, `BusinessDayCalculator.java`, `LoginApprovalEscalationJob.java` (novos)
- `domain/access/login/dto/LoginApprovalRequestOutput.java` (novo — correção pós-review)
- `domain/access/login/specification/LoginApprovalRequestService.java` (novo — correção pós-review)
- `domain/access/login/service/LoginApprovalRequestServiceBean.java` (novo — correção pós-review, regra de negócio movida do usecase)
- `domain/access/login/specification/AuthorityResponseService.java`, `domain/access/login/service/AuthorityResponseServiceBean.java` (`findAllByPermission` — correção pós-review)

**Usecase (`flow-organization-usecase`):**
- `usecase/corporate/department/UpdateDepartmentUseCaseBean.java`, `FindDepartmentUseCaseBean.java`, `FindAllDepartmentUseCaseBean.java`
- `usecase/access/login/DecisionType.java` (novo)
- `usecase/access/login/LoginApprovalRequestApiMapper.java` (novo — reescrito na correção pós-review para trabalhar com `LoginApprovalRequestOutput`, não mais com a entidade)
- `usecase/access/login/DecideLoginApprovalRequestUseCase(Bean).java`, `FindLoginApprovalRequestUseCase(Bean).java`, `FindAllLoginApprovalRequestUseCase(Bean).java`, `FindSystemAccessApproversUseCase(Bean).java` (novos — reescritos na correção pós-review para chamar `*Service`, não `*Repository`)

**API (`flow-organization-api`):**
- `api/delegate/login/LoginApprovalRequestDelegate.java` (novo)

**Infrastructure / Shared:**
- `infrastructure/enumaration/ScosOrganizationPermission.java`
- `messages_organization_permission.properties`, `messages_organization_permission_en.properties`
- `shared/exception/ExceptionCodeError.java`
- `scos_message_organization.properties`, `scos_message_organization_en.properties`

**Boot:**
- `boot/ScosOrganizationApplication.java` (`@EnableScheduling`)

**Testes:**
- `domain/.../login/service/LoginApprovalChainResolverTest.java`, `BusinessDayCalculatorTest.java`, `LoginApprovalEscalationJobTest.java` (novos)
- `domain/.../login/service/LoginServiceBeanTest.java`, `domain/.../department/service/DepartmentServiceBeanTest.java` (atualizados)
- `domain/.../login/service/LoginApprovalRequestServiceBeanTest.java` (novo — correção pós-review, absorve os cenários de negócio que antes estavam no teste do usecase)
- `domain/.../login/service/AuthorityResponseServiceBeanTest.java` (novo — correção pós-review, cobre `findAllByPermission`)
- `usecase/.../login/DecideLoginApprovalRequestUseCaseBeanTest.java`, `FindLoginApprovalRequestUseCaseBeanTest.java`, `FindAllLoginApprovalRequestUseCaseBeanTest.java`, `FindSystemAccessApproversUseCaseBeanTest.java` (reescritos na correção pós-review — mockam `*Service`, não mais `*Repository`)
- `usecase/.../department/UpdateDepartmentUseCaseBeanTest.java` (atualizado)
- `boot/.../department/DepartmentControllerTest.java` (atualizado — cenários `managerId`)
- `boot/.../login/LoginApprovalRequestControllerTest.java` (novo)

**Documentação:**
- `etc/doc/usecase/08-fluxo-cadeia-aprovacao-login.md` (novo — fluxos de criação, consulta, aprovação, rejeição e escalonamento, visão do usuário + visão de classes/métodos)
- `etc/doc/usecase/00-indice-central.md` (linha de navegação para o Documento 08 + nota de desatualização da Seção 1)

## Change Log

| Data | Descrição |
|------|-----------|
| 2026-08-16 | Story implementada (dev-story). Todas as 10 tasks concluídas; suíte completa do monorepo verde (`mvn test`, todos os módulos, incluindo integração via Testcontainers). Ver Debug Log References para as regressões/gaps pré-existentes encontrados e corrigidos durante a implementação. |
| 2026-08-16 | Correção pós-review pedida pelo usuário: `DecideLoginApprovalRequestUseCaseBean` e os demais Use Cases de `access/login` chamavam `*Repository` diretamente (quebra do padrão usecase→domain service→repository). Regra de negócio extraída para o novo domain service `LoginApprovalRequestServiceBean`; `system-access-approvers` passou a usar `AuthorityResponseService.findAllByPermission` em vez do repositório de view direto. Criado `etc/doc/usecase/08-fluxo-cadeia-aprovacao-login.md` com os fluxos de criação/consulta/aprovação/rejeição/escalonamento, visão do usuário e visão de classes/métodos. Suíte completa revalidada verde. |
