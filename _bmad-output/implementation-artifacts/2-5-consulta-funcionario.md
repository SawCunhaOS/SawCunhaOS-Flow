...
6. **Given** sem token, ou com token válido mas sem a permissão `GET_EMPLOYEE` **When** qualquer uma das 2 rotas é chamada **Then** o sistema rejeita com `401` (sem token) ou `403 SCOS-004` (sem permissão) — `GET_EMPLOYEE` já está cadastrada em `ScosOrganizationPermission` (Story 2.1), nenhuma permissão nova é criada nesta story.
