...
- [x] Task 1: `TokenAuthorizationInterceptor` — bug do tipo de exceção + teste (AC: 1, 5, 6)
  - [x] `flow-organization-grpc-boot/pom.xml`: adicionar bloco de teste (`grpc-boot` não tem nenhum hoje), mesmo conjunto exato já usado em `flow-organization-domain/pom.xml`:
    ...
    ```
  - [x] `flow-organization-grpc-boot/.../configuration/interceptor/TokenAuthorizationInterceptor.java:109`: trocar `catch (NoSuchElementException ex)` por `catch (ScosException ex)`; remover `import java.util.NoSuchElementException;`; adicionar `import br.com.sawcunhaos.foundation.core.exception.ScosException;`.
