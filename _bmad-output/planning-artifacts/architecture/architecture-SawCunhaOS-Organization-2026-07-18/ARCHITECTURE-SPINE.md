...
Mapeamento de pacote inalterado: `br.com.sawcunhaos.organization.*`. Módulos hoje vivem em `organization/flow-organization-{domain,usecase,api,infrastructure,boot,grpc-boot,grpc-proto,resources,shared}` + `organization/flow-security-starter` (renomeados de `scos-organization-*`/`scos-security-starter` para monólito modular — reestruturação já concluída, não é decisão desta spine).
