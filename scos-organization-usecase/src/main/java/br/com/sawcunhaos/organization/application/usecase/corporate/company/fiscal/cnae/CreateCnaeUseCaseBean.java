...
/** Implementação de {@link CreateCnaeUseCase}. */
@Service
@RequiredArgsConstructor
@Slf4j
class CreateCnaeUseCaseBean implements CreateCnaeUseCase {

    private final CnaeService cnaeService;

    @Override
    public Cnae execute(@NonNull CreateCnaeRequest createCnaeRequest) {
        log.info("Create cnae: {}", createCnaeRequest.code());

        CnaeInput cnaeInput = CnaeInput.builder()
                .code(createCnaeRequest.code())
                .description(createCnaeRequest.description())
                .build();

        CnaeOutput cnaeOutput = cnaeService.create(cnaeInput);
        return CnaeApiMapper.toApiCnae(cnaeOutput);
    }
}
